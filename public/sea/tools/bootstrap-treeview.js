...
(function($, window, document, undefined) {
	...
	Tree.defaults = {
		...
		expandIcon 			: 'i i-plus2',
		collapseIcon 		: 'i i-minus2',
		nodeIcon 			: 'i i-dot',
	...
	Tree.prototype = {
		...
		_clickHandler : function(event) {
			...
			if ((classList.indexOf('click-expand') != -1)
				...
				if(this.options.highlightSelected) this._setSelectedNode(node);
			}
		},
...
//				console.log(node+id);
				if(id == 8){
//					console.log(node);
					return self._setInitialLevels(node.nodes, level);
				}
		...
		_buildTree : function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				var menuClass = 'badge btn-danger menu';
				var menuIconClass = 'i i-cross2';
				var menuTemplate = $(self._template.menu);
//				console.log(node.status);
				if(parseInt(node.status)){
					menuClass = 'badge btn-primary menu';
					menuIconClass = 'i i-checkmark2';
				}
				menuTemplate.attr('class',menuClass).children('i').attr('class',menuIconClass);
				treeItem.append(menuTemplate).append(self._template.edit).append(self._template.add);
				// Add expand, collapse or empty spacer icons
				// to facilitate tree structure navigation
				if (node._nodes) {
					treeItem.append($(self._template.iconWrapper).append($(self._template.icon).addClass('click-expand').addClass(self.options.expandIcon)));
				} else if (node.nodes) {
					treeItem.append($(self._template.iconWrapper).append($(self._template.icon).addClass('click-collapse').addClass(self.options.collapseIcon)));
				} else {
					treeItem.append($(self._template.iconWrapper).append($(self._template.icon).addClass(self.options.nodeIcon)));
				}

				// Add node icon
				if (!treeItem.find('span.icon')) {
					treeItem.append($(self._template.iconWrapper).append($(self._template.icon).addClass(node.icon ? node.icon : self.options.nodeIcon)));
				}

				// treeItem.append($(self._template.iconWrapper).append($(self._template.icon).addClass(node.icon
				// ? node.icon : self.options.nodeIcon)));
		...
		_template : {
			...
			edit 		: '<span class="badge btn-info edit"><i class="fa fa-edit"></i> Edit</span>',
			menu	 	: '<span class="badge btn-primary menu"><i class="i i-cross2"></i> Menu</span>',
			add 		: '<span class="badge btn-success add"><i class="i i-add-to-list"></i> Add</span>',
		},
